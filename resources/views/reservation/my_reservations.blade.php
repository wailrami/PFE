<x-app-layout>
    ...
    <div class="py-12">
            ...
            @if ($reservations->isEmpty())
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <p class="text-center text-gray-500 dark:text-gray-300">No reservations yet.</p>
                    </div>
                </div>
            @else
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @php
                                    $i = 1;
                                    $nonPeriodicReservations = $reservations->whereNull('periodic_reservation_id');
                                @endphp
                                @foreach ($nonPeriodicReservations as $reservation)
                                    <tr>
        ...
        {{-- add another table for periodic reservations and its status --}}
        @if ($periodicReservations->isNotEmpty())
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    {{-- other colmns : start time, end time, period type --}}
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                        {{ __('Start Time') }}
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                @foreach ($periodicReservations as $reservation)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm
                                            text-gray-900 dark:text-gray-100">{{ $i++ }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 dark:text-gray-100"> <a href="{{route('infrastructure.details',['infrastructure'=> $reservation->infrastructure])}}" > {{ $reservation->infrastructure->name }} </a></div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 dark:text-gray-100">{{ ($reservation->type_period == 'daily')? 'Daily': ($reservation->type_period=='weekly'? 'Weekly': 'Monthly') }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-sm leading-5 font-semibold rounded-full {{ $reservation->etat == 'pending' ? 'bg-orange-500' : ($reservation->etat == 'accepted' ? 'bg-green-600' : 'bg-red-600') }} text-white">
                                                {{ $reservation->etat == 'pending' ? 'Pending' : ($reservation->etat == 'accepted' ? 'Accepted' : 'Rejected') }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            @if ($reservation->etat == 'accepted' || $reservation->etat == 'pending')
                                                @if ($reservation->etat == 'pending')
                                                    <a href="{{ route('reservations.editPeriodic', $reservation) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-600">Edit</a> 
                                                @endif
                                                <form action="{{ route('reservations.cancelPeriodic',$reservation) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-600">Cancel</button>
                                                </form>
